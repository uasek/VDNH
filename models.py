...
def make_index(tokenized_sentence):
    t_slice = tokenized_sentence.target_tokens
    s_slice = tokenized_sentence.source_tokens
    return s_slice.reshape(-1, 1), t_slice

def normalize_along_axis(array):
...
class WordAligner(BaseAligner):
    ...
    def _e_step(self, parallel_corpus: List[TokenizedSentencePair]) -> List[np.array]:
        ...
        posteriors = []
        for sentence in parallel_corpus:
            ind = make_index(sentence)
            posteriors.append(
                normalize_along_axis(self.translation_probs[ind])
            )
        return posteriors
    ...
    def _compute_elbo(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]) -> float:
        ...
        """
        lb = 0
        for j, sentence in enumerate(parallel_corpus):
            ind = make_index(sentence)
            lb -= len(sentence.target_tokens) * np.log(len(sentence.source_tokens))
            lb += np.sum(posteriors[j] * (np.log(self.translation_probs[ind]) - np.log(posteriors[j])))
        return lb
    ...
    def _m_step(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]):
        ...
        for j, sentence in enumerate(parallel_corpus):
            ind = make_index(sentence)
            np.add.at(self.translation_probs, ind, posteriors[j])
            
        self.translation_probs /= np.sum(self.translation_probs, axis=1).reshape(-1, 1)
        lb = self._compute_elbo(parallel_corpus, posteriors)
        return lb
    ...
    def align(self, sentences):
        ...
        for sentence in sentences:
            ind = make_index(sentence)
            posterior = normalize_along_axis(self.translation_probs[ind])
            results.append(
                list(zip(
                    np.argmax(posterior, axis=0) + 1,
                    np.arange(len(sentence.target_tokens)) + 1
                ))
            )
        return results
